...
async fn main() {
    ...
    let _ = futures::future::join_all(records.iter().map(|record| async {
        ...
        if stream.is_err() {
            error!("[{}@{}:{}] Couldn't reach node", hex::encode(&remote_id), ip, port);
        ...
        ******************/
        ...
        if result.is_err() {
            ...
            error!("[{}@{}:{}] Unknown error", hex::encode(&remote_id), ip, port);
            let _result = postgres_client.execute(&delete_statement, &[&remote_id]).await.unwrap();
            return
        }
        ...
        ******************/
        ...
        info!("[{}@{}:{}] ACK message received", hex::encode(&remote_id), ip, port);
        ...
        ******************/
        ...
        let (mut ingress_aes, mut ingress_mac, mut egress_aes, mut egress_mac) = utils::setup_frame(remote_nonce, nonce, ephemeral_shared_secret, remote_data, init_msg, h_nonce);
    
        info!("[{}@{}:{}] Frame setup done !", hex::encode(&remote_id), ip, port);
    
        info!("[{}@{}:{}] Received Ack, waiting for Header", hex::encode(&remote_id), ip, port);
        ...
        ******************/
        ...
        if uncrypted_body[0] == 0x01 {
            ...
            error!("[{}@{}:{}] Disconnect {}", hex::encode(&remote_id), ip, port, hex::encode(uncrypted_body[1..].to_vec()));
            return
        }
        ...
        ******************/
        ...
        info!("[{}@{}:{}] Sending HELLO message", hex::encode(&remote_id), ip, port);
        // Create Hello
        ...
        ******************/
        ...
        info!("[{}@{}:{}] Sending STATUS message", hex::encode(&remote_id), ip, port);
        ...
        ******************/
        ...
        info!("[{}@{}:{}] Handling STATUS message", hex::encode(&remote_id), ip, port);
        let uncrypted_body = utils::read_message(&mut stream, &mut ingress_mac, &mut ingress_aes);
        if uncrypted_body.is_err() {
            error!("[{}@{}:{}] Time out", hex::encode(&remote_id), ip, port);
            return
        ...
        info!("[{}@{}:{}] network_id = {:?}", hex::encode(&remote_id), ip, port, &status.network_id);

        let cap : Vec<(String, u32)> = serde_json::from_str(&capabilities).unwrap();
        let _result = postgres_client.execute(&update_statement, &[&(status.network_id as i64), &serde_json::to_value(&cap).unwrap(), &hello_message.client, &remote_id]).await.unwrap();
    })).await;
